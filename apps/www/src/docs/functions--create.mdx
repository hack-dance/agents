...
The function creator is a light-weight utility that allows you to create consistent function definitions for your agents. It takes a zod schema and an execute function, and returns a definition that you can pass to open ai directly or through the agent creator.

**Definition**

<span>`@/ai/fns/greetFn.ts`</span>
...
<span>`@/ai/agents/greetAgent.ts`</span>
...
<span>`@/app/api/greet/route.ts`</span>
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    executeFunctionFromResponse(response, greeterFns)
